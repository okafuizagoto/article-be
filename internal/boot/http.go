...
import (
	...
	// "article-be/internal/data/auth"

	// "article-be/pkg/firebaseclient"

	"article-be/pkg/tracing"
	"log"
	"net/http"
	...
	// "article-be/pkg/trace"

	articleData "article-be/internal/data/article"
	articleServer "article-be/internal/delivery/http"
	articleHandler "article-be/internal/delivery/http/article"
	articleService "article-be/internal/service/article"

	articleStockData "article-be/internal/data/stock"
	articleStockService "article-be/internal/service/stock"
	// articleStockData "article-be/internal/data/stock"
	// pushNotifData "article-be/internal/data/pushnotif"
	// pushNotifHandler "article-be/internal/delivery/http/pushnotif"
	// pushNotifService "article-be/internal/service/pushnotif"
)
...
func HTTP() error {
	...
	sdst := articleStockData.New(db, nil, nil, rdb, tracer, zlogger)
	ssst := articleStockService.New(sdst, tracer, zlogger)

	// Diganti dengan domain yang anda buat
	sd := articleData.New(db, tracer, zlogger)
	// ss := articleService.New(sd, ad, tracer, zlogger)
	ss := articleService.New(sd, tracer, zlogger)
	sh := articleHandler.New(ss, ssst, tracer, zlogger)

	// sdprod := articleData.New(dbprod, tracer, zlogger)
	// ssprod := articleService.New(sdprod, tracer, zlogger)

	// sha := authHandler.New(ss, tracer, zlogger)
	// sh := articleHandler.New(ss, tracer, zlogger)
